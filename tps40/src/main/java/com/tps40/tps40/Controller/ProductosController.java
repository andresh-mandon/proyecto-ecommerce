...
private String guardarImagen(MultipartFile imagen) throws IOException {
    log.info("Entrando al método guardarImagen()");
    Path uploadDir = Paths.get(uploadPath);
    log.info("Ruta de uploads en guardarImagen: {}", uploadDir.toAbsolutePath());
    ...
    log.info("Archivo '{}' guardado en: {}", nombreArchivo, destino.toAbsolutePath());
   ...
   public String actualizarProducto(
       ...
       log.info("Intentando actualizar el producto con ID: {}", id);
       log.info("Nombre del producto desde el formulario: {}", producto.getNombre());
       log.info("Nombre del archivo subido: {}", imagenFile != null ? imagenFile.getOriginalFilename() : "null");
       log.info("uploadPath configurado como: {}", uploadPath);
   
       try {
           Productos productoExistente = productoService.obtenerProductoPorId(id);
           log.info("Imagen existente antes de la actualización: {}", productoExistente.getImagen());
   
           String nuevoNombreImagen = productoExistente.getImagen(); // Inicializar con el nombre existente
   
           // Manejo de la imagen
           if (imagenFile != null && !imagenFile.isEmpty()) {
               log.info("Se recibió un nuevo archivo de imagen: {}", imagenFile.getOriginalFilename());
               ...
               if (productoExistente.getImagen() != null && !productoExistente.getImagen().equals("default-product.png")) {
                   ...
                   try {
                       boolean deleted = Files.deleteIfExists(rutaAnterior);
                       log.info("¿Se eliminó la imagen anterior '{}'? {}", productoExistente.getImagen(), deleted);
                   } catch (IOException e) {
                       log.error("Error al eliminar la imagen anterior '{}': {}", productoExistente.getImagen(), e.getMessage());
                   }
               }
               // Guardar nueva imagen
               String nombreImagenGuardada = guardarImagen(imagenFile);
               nuevoNombreImagen = nombreImagenGuardada;
               log.info("Nueva imagen guardada con el nombre: {}", nuevoNombreImagen);
           } else {
               log.info("No se subió una nueva imagen, se mantiene la existente.");
           }
   
           producto.setImagen(nuevoNombreImagen);
           log.info("Nombre de la imagen establecido en el producto: {}", producto.getImagen());
   
           // Actualizar producto
           productoService.actualizarProducto(id, producto);
           log.info("Producto actualizado (se espera que la imagen sea: {})", producto.getImagen());
